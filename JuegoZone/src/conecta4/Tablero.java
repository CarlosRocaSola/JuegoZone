...
public class Tablero {
    ...
    /**
     * Inserta una ficha en el tablero, en la columna especificada.
     * Devuelve:
     * 0 si la ficha se inserta correctamente.
     * 1 si la columna en la que se intenta colocar est� llena.
     * -1 en caso de error.
     * @param columna Indica la columna en la que se coloca la ficha
     * @param jugador Indica el jugador que est� jugando, 1 para jugador 1, 
     * 2 para jugador 2
     */
    ...
    public int ponerFicha (int columna, int jugador)
    {
    	...
    	//Comprueba que la columna pasada como par�metro sea correcta
    	if ((columna < 0) || (columna >= m_numColumnas))
    		return -1;
    	
    	//Comprueba la primera casilla libre de es columna (si tiene)
    	...
    	//Si cuando sale del for no ha encontrado ninguna casilla libre es que la columna est� llena
    	if(!encontrado)
    		return 1;
    	else
    		return 0;
    ...
    /**
     * Comprueba si la columna que se le pasa como par�metro es correcta para una jugada.
     * Devuelve  la fila en la que le se colocar�a la ficha y -1 en caso de error.
     * @param columna
     * @return
     */
    public int comprobarColumna(int columna)
    {
    	...
    	//Comprueba la primera casilla libre de esa clomna (si tiene)
    	for (int i = 0; (i<m_numFilas && !encontrado); i++)
    	{
    		if(m_tablero[i][columna] == 0)
    		{
