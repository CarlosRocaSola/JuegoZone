...
public class Juego {
	...
	/**
	 * crea un thread donde se ejecutar� JugadorMaquina para que calcule la jugada
	 * @param jugador JugadorMaquinq que esta jugando 
	...
	public int jugadaMaquina(JugadorMaquina jugador){
		...
		if(m_turno == jugador.m_jugador){
			...
			//Si la colocaci�n de la ficha ha sido correcta 
			if(resultado == 0){
				//Compruebasi ha ganado  el jugador 
				if(m_tablero.cuatroEnRaya()!=0){
					m_mensaje = 2;
					m_jugando = false;
				}
	...
	/**
	 * Cambia el turno de jugador .se utiliza despu�s de cada jugada  
	 */
